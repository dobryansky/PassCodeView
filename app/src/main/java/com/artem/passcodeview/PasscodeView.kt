...
import FillAndStrokeColorChangeAnimation
import FillColorChangeAnimation
import ShakeAnimator
import SpringAnimator
...
class PasscodeView @JvmOverloads constructor(
    ...
    companion object {
        ...
        private const val DEFAULT_INPUT_COLOR = R.color.green
    ...
    private val array = context.obtainStyledAttributes(attrs, R.styleable.PasscodeView)
    var passwordCount = array.getInteger(R.styleable.PasscodeView_password_count, DEFAULT_CIRCLE_COUNT)
        set(value) {
            field = value
            addCircleView(passwordCount)
        }
    private val radius = array.getDimension(R.styleable.PasscodeView_password_radius, DEFAULT_RADIUS)
    private val betweenMargin =
        array.getDimensionPixelOffset(R.styleable.PasscodeView_password_between_margin, DEFAULT_BETWEEN_MARGIN)
    private val inputColor = array.getColor(R.styleable.PasscodeView_password_input_color, resources.getColor(DEFAULT_INPUT_COLOR))
    private val notInputColor = array.getColor(R.styleable.PasscodeView_password_input_color, DEFAULT_NOT_INPUT_COLOR)
    private val correctColor = array.getColor(R.styleable.PasscodeView_password_correct_color, DEFAULT_CORRECT_COLOR)
    private val incorrectColor =
        array.getColor(R.styleable.PasscodeView_password_incorrect_color, DEFAULT_INCORRECT_COLOR)
    private val correctAnimationDuration =
        array.getInteger(R.styleable.PasscodeView_password_correct_duration, DEFAULT_CORRECT_ANIMATION_DURATION)
            .toLong()
    private val incorrectAnimationDuration =
        array.getInteger(R.styleable.PasscodeView_password_correct_duration, DEFAULT_INCORRECT_ANIMATION_DURATION)
            .toLong()
    private val colorChangeAnimationDuration =
        array.getInteger(
            R.styleable.PasscodeView_password_color_change_duration,
            DEFAULT_COLOR_CHANGE_ANIMATION_DURATION
        )
            .toLong()
    private val inputAndRemoveAnimationDuration =
        array.getInteger(
            R.styleable.PasscodeView_password_input_and_remove_duration,
            DEFAULT_INPUT_AND_REMOVE_ANIMATION_DURATION
        ).toLong()
    private val correctTop = array.getDimension(R.styleable.PasscodeView_password_correct_top, DEFAULT_CORRECT_TOP)
    private val correctBottom =
        array.getDimension(R.styleable.PasscodeView_password_correct_bottom, DEFAULT_CORRECT_BOTTOM)
    private val incorrectMaxWidth =
        array.getDimension(R.styleable.PasscodeView_password_incorrect_max_width, DEFAULT_INCORRECT_MAX_WIDTH)
                ...
                addListener(object : Animator.AnimatorListener {
                    override fun onAnimationEnd() {
                        if (index == passwordCount - 1) {
                            this@PasscodeView.actionListener?.onEndJudgeAnimation()
                        }
                    }
                })
